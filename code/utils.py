'''
    Description:
        reading the datesets from ../data folder to generate the graphs/features and plots.
'''
from log import Log
import torch
...
debug_log = Log('debug', file_path='../log/debug.txt')
debug_logger = debug_log.get_log()
...
def read_meta_datasets(window):
    ...
    # ------------------ Italy
    os.chdir("Italy") # label中包含每日的阳性数量
    ...
    #debug_logger.info('dates len:{}'.format(len(dates)))
    ...
    # ------------------------- Spain
    os.chdir("../Spain")
    labels = pd.read_csv("spain_labels.csv")  # labels - DataFrame
    ...
    for date in dates:
        ...
        for row in d.iterrows():
            G.add_edge(row[1][0], row[1][1],
                       weight=row[1][2])  # add edges which are composed of start destination and weights
        Gs.append(G)

    return Gs

def generate_graphs_air(dates, country):
    Gs = []
    for date in dates:
        d = pd.read_csv("graphs/" + country + "_" + date + ".csv", header=None)
        G = nx.DiGraph()
...
if __name__ == "__main__":
    ...
    meta_labs, meta_graphs, meta_features, meta_y = read_meta_datasets(tmp_window)
    print(meta_labs, meta_graphs, meta_features, meta_y)
