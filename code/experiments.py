...
from models import MPNN_LSTM, LSTM, MPNN, prophet, arima, MPNN_trans
from log import Log
    ...
    for args.model in ["MPNN_LSTM"]:
        ...
        for shift in list(range(0, args.ahead)):
            ...
            for test_sample in range(args.start_exp, n_samples - shift):  #
                ...
                while (not stop):  #
                    if (args.model == "LSTM"):
                        ...
                        model = MPNN_trans(nfeat=nfeat, nhid=args.hidden, nout=1, n_nodes=n_nodes,
                                           window=args.graph_window, dropout=args.dropout, num_layers=1).to(device)
                    ...
                    for epoch in range(args.epochs):
                        ...
                        output, val_loss = model_test(adj_val[0], features_val[0], y_val[0])
                ...
                output, loss = model_test(adj_test[0], features_test[0], y_test[0])
                ...
                # Print results
                # logger.info("test error=", "{:.5f}".format(error))
